...
printf_loop:
    lbu    $a0, 0($s0)
    beqz    $a0, printf_ret
    beq     $a0, '\\', printf_format #if $a0 contains escape slashes '\\', jump to printf_format
...
printf_format:
    ...
    beq     $a0, 'd', printf_int
    beq    $a0, 's', printf_str
    beq $a0, '%', printf_esc #if $a0 contains escape slashes, jump to printf_esc
printf_int: 
    lw    $a0, 0($t0)
    li    $v0, 1
    syscall
...
printf_str:
    ...
    j     printf_last
printf_esc:
        li $v0, 11 #prep % character for print
        syscall #system call
        j printf_last #go to printf_last in to allow rest of string to print
printf_last:
